...
struct BaseRequest(FromReq, Copyable, Movable):
...
trait RouteHandlerTrait(Movable, Copyable):
...
struct RouterBase[is_main_app: Bool = False](HTTPService, Copyable, Movable):
    ...
    fn _find_route(self, partial_path: String, method: String, depth: Int = 0) raises -> Int:
        ...
        if sub_router_name in self.sub_routers:
            return self.sub_routers[sub_router_name]._find_route(
